...
<script type="text/javascript">
	$(function(){
				...
				$.ajax({
					...
					success:function (data) {
							...
							$.each(data,function (i,n) {
								html += '<tr>'
								html += '<td><input type="radio" name="check" value="'+n.id+'" /></td>'
		...
		$("#convertBtn").click(function () {
			//传统请求，执行线索转换的操作， 响应回线索列表项
			//根据客户有没有选择复选框 判断是否创建交易
			if($("#isCreateTransaction").prop("checked")){
				//alert("需要创建交易"); 需要的话 除了为后台传递clueID 还有其他几个
				//金额，预计成交日期，交易名称，阶段，市场活动源
				<%--window.location.href = "workbench/clue/convert.do?clueId=${param.clueId}&money=xx&expectedDate=xx&stage=xx&activityId=xx}";--%>
				//以上传递参数方式很麻烦，表单一旦扩充，挂载的参数有可能会超出浏览器地址栏上限
				//form表单提交 参数不用手动挂载，(表单中写name属性),提交表单 post请求
				$("#tranForm").submit();
			}else {
				//alert("不需要创建交易");  传递一个clueId
